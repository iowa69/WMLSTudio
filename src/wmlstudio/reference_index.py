...
def _entry(path: Path) -> dict:
    ...
    declared = str(metadata.get("type", "")).casefold()
    kind = ("cgmlst" if count > CGMLST_LOCUS_THRESHOLD or declared == "cgmlst"
            else "mlst" if count else "unknown")
    return {"path": str(path), "id": path.name, "name": str(metadata.get("name") or path.name),
            ...
            "label": label, "locus_count": count, "kind": kind,
            "organism_basis": _organism_basis(metadata, organism)}


def scheme_entries(scheme_paths, *, cancelled=None) -> list[dict]:
