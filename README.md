...
Go to the [**Releases**](https://github.com/iowa69/WMLST/releases/latest) page and download

```
WMLST-1.0.0-win64-setup.exe
```

Double-click it. It installs into your own user folder, so **no administrator password
is needed**.
...
### Step 2 — Start WMLST and let it finish setting up

Open **WMLST** from the Start menu. The first time it runs it will offer to download
**NCBI BLAST+** (about 143 MB, from NCBI, over HTTPS, checksum-verified). Say yes and
let it finish; it only happens once. Then it builds a search index from the bundled
database, which takes about ten seconds.

### Step 3 — Drop your contigs on the window and press *Analyse*

Drag one or more assembly files onto the window — `.fasta`, `.fa`, `.fna`, `.gbk`,
`.embl`, and any of them `.gz`, `.bz2` or `.zip` — and click **Analyse**. Results
appear in a table you can sort, copy, and export as TSV, CSV, JSON or a
self-contained HTML report you can email to a colleague.
...
2. **Jolley KA, Bray JE, Maiden MCJ (2018)** "Open-access bacterial population genomics:
   ...
   *Wellcome Open Research* **3**:124.
   ...
   PMID **30345391**
3. **WMLST 1.0.0**, IOWA-Tech — Giovanni Lorenzin, <https://github.com/iowa69/WMLST>
